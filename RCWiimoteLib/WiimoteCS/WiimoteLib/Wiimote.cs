...
namespace WiimoteLib
{
	...
	public class Wiimote : IDisposable
	{
        ...
        // current state of the Wiimotes Connection
			...
			if(HIDImports.HidD_GetAttributes(mHandle.DangerousGetHandle(), ref attrib))
			{
				...
				if(attrib.VendorID == VID && attrib.ProductID == PID)
				{
                    ...
                    try
                    {
                        ...
                        string tstr = "Read the wiimote calibration: " + "X0: " + mWiimoteState.AccelCalibrationInfo.X0.ToString() + " XG: " + mWiimoteState.AccelCalibrationInfo.XG.ToString();
		...
		public void Disconnect()
		{
            ...
            mWiimoteState.ConnectionState = ConnectionState.NotConnected;
        ...
        /// <param name="reqReadSize">The requested read size, for when parsing read data</param>
		/// <returns>Returns a boolean noting whether an event needs to be posted</returns>
		private bool ParseInputReport(byte[] buff, short reqReadSize)
		{
			...
			switch(type)
			{
				...
				case InputReport.ReadData:
					ParseButtons(buff);
                    ParseReadData(buff, reqReadSize);
					break;
...
//					mWiimoteState.ClassicControllerState.AccelCalibrationInfo.MaxTriggerR = (byte)(buff[15] >> 3);
		...
		/// <summary>
		/// Decodes data sent from the extension to the Wiimote
		/// </summary>
		...
		private void ParseIR(byte[] buff)
		{
            ...
            // This code adjusts the raw position to take account of the focal length
		...
		/// </summary>
		/// <param name="buff">Data buffer</param>
        /// <param name="reqReadSize">The requested read size</param>
		private void ParseReadData(byte[] buff, short reqReadSize)
		{
			if((buff[3] & 0x08) != 0)
        ...
        private byte GetRumbleBit()
        {
            ...
            Debug.WriteLine("Going to read the rumble bit.");
            //lock (this)
            //{
            if (mWiimoteState.Rumble)
            {
                buff = 0x01;
            }
            else
		...
		public void GetStatus()
		{
            ...
            Debug.WriteLine("Sucessfully read RumbleBit in GetStatus() it was: " + Buff[1].ToString());
            ...
            {
                Debug.WriteLine("Line: 1067: Timed out waiting for status report in GetStatus()");
        ...
        /// </summary>
        /// <param name="buff">Data buffer</param>
        private byte[] ClearReport(byte[] buff)
        {
		...
		/// <summary>
		/// Write a report to the Wiimote using the contents of the global data buffer mBuff
		/// </summary>
		private void WriteReport()
		{
			Debug.WriteLine("WriteReport mBuff Contents: " + mBuff[0].ToString("x"));
			if(mAltWriteMethod)
        ...
        /// <summary>
        /// Write a report to the Wiimote using the contents a local data buffer passed in
        /// </summary>
        /// <param name="buff">Data buffer</param>
		...
		public byte[] ReadData(int address, short size)
		{
            ...
            {
                Debug.WriteLine("Line 1198: Error reading data from Wiimote...is it connected?");
		...
		{
            byte[] writeBuff = new byte[REPORT_LENGTH];

			//ClearReport();

            writeBuff[0] = (byte)OutputReport.WriteMemory;
            writeBuff[1] = (byte)(((address & 0xff000000) >> 24) | GetRumbleBit());
            writeBuff[2] = (byte)((address & 0x00ff0000) >> 16);
            writeBuff[3] = (byte)((address & 0x0000ff00) >> 8);
            writeBuff[4] = (byte)(address & 0x000000ff);
            writeBuff[5] = size;

            Array.Copy(buff, 0, writeBuff, 6, size);

            WriteReport(writeBuff);
		}
